...
namespace FC_ParkourSystem
{
    public class ClimbController : MonoBehaviour
    {
        ...
        //public Vector3 handOffsets  { get => animator.GetFloat("freeHang") < 0.5f ? bracedHandOffsets : freehangHandOffsets; }
        ...
        private Vector3 getFootIK(Vector3 rayStartPos, Vector3 rayDir)
        {
            ...
            bool isWall = Physics.Raycast(rayStartPos, rayDir, out RaycastHit hitInfo, footIkRayLength, envScanner.ObstacleLayer);
        ...
        public void HandleUpdate()
        {
            if (!parkourController.IsHanging && !parkourController.InAction)
            {
                ...
                else if (input.Drop && !parkourController.InAction)
                {
                    if (envScanner.DropLedgeCheck(transform.forward, out ClimbLedgeData ledgeData))
                    {
                        currentLedge = ledgeData.ledgeHit.transform;
                        var newPoint = GetNearestPoint(currentLedge, ledgeData.ledgeHit.point, false, obstacleCheck: false);
                        ...
                        if (CheckWall(currentPoint).Value.isWall)
                        {
                            ...
                            StartCoroutine(JumpToLedge(currentPoint.transform, "DropToHang", 0.50f, 0.90f, rotateToLedge: true, matchStart: AvatarTarget.LeftFoot));
                        }
                        else
                        {
                            animator.SetFloat("freeHang", 1);
                            StartCoroutine(JumpToLedge(currentPoint.transform, "DropToFreeHang", 0.50f, 0.89f, rotateToLedge: true, matchStart: AvatarTarget.LeftFoot));
                        }
                    }
                }
            }
            ...
            {
                var moveInput = input.DirectionInput;
                float h = moveInput.x;
                float v = moveInput.y;
                ...
                //animator.SetFloatMathf.Sign(value) * 0.1f + value("jumpBackDirection", Mathf.Sign(value) * 0.1f + value);
                if (Mathf.Abs(value) < 0.2f) value = 0; //limit movement
                else
                    animator.SetBool("BackJumpMode", true);

                animator.SetFloat("BackJumpDir", Mathf.Clamp(value, -1, 1), 0.2f, Time.deltaTime * 2);
                //}
                if (input.JumpFromHang || (input.Jump && moveDir == Vector3.zero))
                {
                    if (moveDir != Vector3.zero || (input.Jump && moveDir == Vector3.zero))
                    {
                ...
                else if (moveInput != Vector2.zero && !parkourController.InAction)
                {
                    ...
                    {
                        if (pressInputToClimb && input.Jump && moveInput.y > .6f && moveInput.x == 0 && animator.GetFloat("freeHang") < 0.5f)
                        {
                            StartCoroutine(DoClimbingAction("Bracedhang Try Jump Up"));
                            parkourController.IsHanging = false;
                            isFalling = true;
                            animator.SetBool("IsGrounded", false);
                            return;
                        ...
                        if (distance < 1f)
                        {
                            if (distanceY > 0.6f && animator.GetFloat("freeHang") > 0.5f)
                                return;
                        }
                        else
                        {
                            if (distanceY > 0f && animator.GetFloat("freeHang") > 0.5f)
                                return;
                        }
        ...
        public IEnumerator ClimbWhenFalling()
        {
            ...
            parkourController.CurrentCameraShakeAmount = 0.01f;
        ...
        public bool ClimbToPoint(Transform currentLedge, Vector3 point, string animName = null, bool combo = false, Vector3? checkDirection = null)
        {
        ...
        public void DoShimmy(float normalTime, bool mirror = false, bool HangTypeChange = false)
        {
            ...
            //if (animator.GetFloat("freeHang") < 0.5f) // to disable it in free hang
            //{
            //    rightFoot.IKPoint = Vector3.Slerp(rightFoot.previousIkVecPoint, rightFoot.currentIkVecPoint, rightFoot.lerpValue);
            //    leftFoot.IKPoint = Vector3.Slerp(leftFoot.previousIkVecPoint, leftFoot.currentIkVecPoint, leftFoot.lerpValue);
            //}
        ...
        ClimbPoint GetNearestPoint(Transform ledge, Vector3 hitPoint, bool checkAngle = true, Vector3? checkDirection = null, bool obstacleCheck = true, Vector3? predictiveMaxHeight = null)
        {
            ...
            foreach (var point in points)
            {
                ...
                if (obstacleCheck)
                {
                    if (Physics.Linecast(transform.position + Vector3.up, pointForward, envScanner.ObstacleLayer))
                        continue;
                }
