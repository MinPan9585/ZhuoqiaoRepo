...
namespace FC_ParkourSystem
{
    ...
    public class ParkourController : MonoBehaviour
    {
        ...
        [field: Tooltip("Enables vertical jump")]
        public bool enableVerticalJump = false;
        ...
        bool prevControlVal;
        float minDistanceFromLedgeForJumping = .5f;     // The character should only jump when he's close to the ledge. This is the distance from ledge at which he will jump
        ...
        private void Update()
        {
            CheckVertcalJumpInput();
        }

        #region Vertical Jump
        void CheckVertcalJumpInput()
        {
            if (input.JumpKeyDown && player.IsGrounded)
                canJump = true;
            else if (ControlledByParkour)
                canJump = false;
        }
        void VerticalJump()
        {
            ...
            if (!headHit && enableVerticalJump && player.IsGrounded && canJump)
            {
                StartCoroutine(player.HandleVerticalJump());
                canJump = false;
            }
        }
        ...
        void HandleUpdate()
        {
            if (player.PreventParkourAction)
                return;
            ...
            if (input.Drop && player.MoveDir != Vector3.zero && !InAction && player.IsGrounded && isOnLedge)
            {
                ...
                {
                    StartCoroutine(DoAction("Jump Down", onComplete: () => player.OnEndParkourAction()));
                    isOnLedge = false;
                    return;
                }
            }
                ...
                if (jumpData != null)
                {
                        ...
                        && (!jumpData.jumpPointFound || (jumpData.jumpPointFound && Vector3.Distance(jumpData.climbHitData.point, transform.position) < Vector3.Distance(jumpData.rootPosition, transform.position))))
                    {
                        ...
                        if (!jumpData.hasLedge || Vector3.Distance(transform.position, jumpData.pointBeforeledge) < minDistanceFromLedgeForJumping)
                            if (climbController.ClimbToPoint(jumpData.climbHitData.transform, jumpData.climbHitData.point,checkDirection: direction))
                                return;
                    }
        ...
        void HandleParkourEvents()
        {
            ...
            {
                player.OnStartParkourAction();
                prevControlVal = true;
            }
            else if (!ControlledByParkour && prevControlVal)
            {
                player.OnEndParkourAction();
        ...
        {
            if (matchFootToTarget)
            {
                //var footPos = animator.GetBoneTransform(mirrorJump ? HumanBodyBones.LeftFoot : HumanBodyBones.RightFoot).position;
                //var offset = _jumpPoint.rootPosition - footPos;
                //var targetRootPos = animator.rootPosition + offset + Vector3.up * 0.15f;

                ////transform.position = Vector3.Lerp(transform.position, targetRootPos, footMatchWeight);
                //transform.position = Vector3.MoveTowards(transform.position, targetRootPos, 5f * Time.deltaTime);
            }

            if (player.UseRootMotion)
            {
                //animator.ApplyBuiltinRootMotion();
                //if(animator.deltaPosition.magnitude > 0.0001215268)
        ...
        {
            //var footPos = animator.GetBoneTransform(mirrorJump ? HumanBodyBones.LeftFoot : HumanBodyBones.RightFoot).position;
            //var offset = jumpPoint.footPosition - footPos;
            //var targetRootPos = animator.rootPosition + offset + Vector3.up * 0.15f;
            //jumpPoint.footPosition = targetRootPos;
                    ...
                    if (isGrounded)
                    {
                        ...
                        if (isFalling)
                        {
                            ...
                            if (jumpPoint.hasSpaceToLand)
                            {
                                ...
                                CurrentCameraShakeAmount = Mathf.Clamp((startPos.y - jumpPoint.rootPosition.y) * 0.003f, 0f, 0.05f);
                                CameraShakeDuration = .7f;
                                if (!hasSpaceForRoll && heightHiting)
                                {
                                    CameraShakeDuration = 1.2f;
        ...
        public IEnumerator DoPredictiveClimb(ClimbPoint point, int climbTransition, string animName = null, float crossFadeTime = 0.2f)
        {
            ...
            while (timer <= temp)
            {
                ...
                else
                {
                    ...
                    if (camShake)
                    {
                        CurrentCameraShakeAmount = Mathf.Clamp((startPos.y - transform.position.y) * 0.003f, 0f, 0.05f);
                    ...
                    if (climbController.currentPoint.transform.parent.CompareTag("SwingableLedge") && comboTime >= (animState.length - transitionTime) * (0.3f)) // peak Time percentage
                    {
                        ...
                        if ((DoCombo && player.MoveDir != Vector3.zero) || DoQuickCombo)
                        {
                            ...
                            var moveDir = ((input.DirectionInput.x * cam.right) + (input.DirectionInput.y * cam.forward)).normalized;
                            ...
                            if (jumpData != null)
                            {
                                if (jumpData.isClimbable && (!jumpData.jumpPointFound || (jumpData.jumpPointFound && jumpData.climbHitData.point.y > jumpData.rootPosition.y)))
                                {
                                    ...
                                    {
                                        if (climbController.ClimbToPoint(jumpData.climbHitData.transform, jumpData.climbHitData.point, combo: true))//transform.position);
                                            yield break;
                                    }
                                }
                    ...
                    else if (input.JumpFromHang && animator.GetFloat("freeHang") > 0.5f) DoCombo = true;
                    else if (input.Jump && animator.GetFloat("freeHang") > 0.5f) DoQuickCombo = true;
            ...
            public void HandlePredictiveBackJump(Vector3 checkDir)
        {
            ...
            if (jumpData != null)
            {
                ...
                if (jumpData.isClimbable && (!jumpData.jumpPointFound || (jumpData.jumpPointFound && jumpData.climbHitData.point.y > jumpData.rootPosition.y)))
                {
                    if (jumpData.climbHitData.point.y - transform.position.y < 2f)
                    {
                        animator.SetFloat("BackJumpDir", 0);
                        animator.SetBool("BackJumpMode", false);
                        if (climbController.ClimbToPoint(jumpData.climbHitData.transform, jumpData.climbHitData.point, checkDirection: checkDir))//transform.position);
                            return;
                    }
                }
                if (jumpData.jumpPointFound)
        ...
        public (Vector3, Vector3) LedgeMovement(Vector3 currMoveDir, Vector3 currVelocity)
        {
            ...
            if (animator.GetFloat("idleType") > 0.5f)
            {
                xOffset = 0.2f;
